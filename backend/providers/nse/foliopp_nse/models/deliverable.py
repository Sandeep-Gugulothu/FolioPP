"""NSE Deliverable Position Data Model."""
...
from foliopp_core.provider.standard_models.nse_models import (
    DeliverablePositionData,
    NSESymbolDateRangeQueryParams,
)

# NSE CSV column names (stripped) -> our standard field names
_COL_MAP = {
    "Symbol": "symbol",
    "Series": "series",
    "Date": "date",
...
# Turnover column has ₹ symbol — match by prefix
...
class NSEDeliverableQueryParams(NSESymbolDateRangeQueryParams):
    pass


class NSEDeliverableData(DeliverablePositionData):
...
class NSEDeliverableFetcher(
    Fetcher[NSEDeliverableQueryParams, list[NSEDeliverableData]]
):
    @staticmethod
    def transform_query(params: dict[str, Any]) -> NSEDeliverableQueryParams:
        return NSEDeliverableQueryParams(**params)

    @staticmethod
    def extract_data(
        query: NSEDeliverableQueryParams,
        ...
        **kwargs,
        ...
        for start, end in date_range_chunks(from_date, to_date):
            ...
            if resp.status_code != 200 or not resp.content:
                continue
            try:
                # Use BytesIO + utf-8-sig to handle BOM
        ...
        if not frames:
            raise EmptyDataError(f"No deliverable data for {query.symbol}")
    ...
    def transform_data(
        ...
        data: list[dict],
        **kwargs,
    ) -> list[NSEDeliverableData]:
        results = []
        for row in data:
            mapped: dict = {}
            for raw_col, val in row.items():
                col = str(raw_col).strip()
                # Match turnover by prefix (column has ₹ symbol)
                ...
                if std is None:
                    continue
                # Numeric fields
                if std in ("prev_close", "open", "high", "low", "close", "avg_price",
                           "total_traded_qty", "trades", "deliverable_qty", "pct_delivery"):
                    mapped[std] = _clean_num(val)
                else:
                    mapped[std] = str(val).strip() if val not in (None, "") else None

            mapped.setdefault("symbol", query.symbol)
            try:
                results.append(NSEDeliverableData.model_validate(mapped))
            except Exception:
                continue
        return results
